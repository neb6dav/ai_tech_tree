...
function main() {
  ...
  fs.writeFileSync(htmlPath, injectNetworkAssets(html, layout, bundle), 'utf8');
  ...
  if (refreshedLayout !== firstLayout) {
    ...
    fs.writeFileSync(htmlPath, injectNetworkAssets(refreshedHtml, refreshedLayout, bundle), 'utf8');
    run(process.execPath, [path.join(ROOT, 'generate-knowledge-graph.js')]);
  }
...
module.exports = { injectNetworkAssets, injectScriptBody, safeInlineJson };
